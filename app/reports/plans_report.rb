# encoding: utf-8
class PlansReport < Prawn::Document

  def set_rep(company, report)
    @company = Company.find(company)
    @report = @company.plans.find(report)
  end

  attr_accessor :company, :report
  # ширина колонок
  Widths = [290, 50, 50, 50, 50, 50]
  # заглавия колонок
  Headers = ['Стаття', 'Код рядка', 'По плану', 'Фактично', 'Відхилення (+, -)', 'Виконання (%)']

  def row(line_name, line_code, value1, value2, value3, value4)
    row = [line_name, line_code, value1, value2, value3, value4]
    make_table([row]) do |t|
  ...
  def to_pdf
    ...
    move_down(18)
    text "Фінансовий план", :size => 15, :style => :bold, :align => :center
    move_down(10)
    text "на  #{@report.period.strftime("%d.%m.%Y")}", :size => 15, :style => :bold, :align => :center
    ...
    items = @values.each do |item|
      data << row(item.line.name_for_select_tag, item.line.code, item.value1, item.value2, item.value1, item.value2)
    end
