# encoding: utf-8
class CompaniesReport < Prawn::Document

  def set_rep(company, report)
    @company = Company.find(company)
    @report = @company.balances.find(report)
  end
  ...
  Widths = [330, 70, 70, 70]
  # заглавия колонок
  Headers = ['Стаття', 'Код рядка', 'На початок звітного періоду', 'На кінець звітного періоду']
  ...
  def to_pdf
    ...
    move_down(18)
    text "Баланс", :size => 15, :style => :bold, :align => :center
    move_down(10)
