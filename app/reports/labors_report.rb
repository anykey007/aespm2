# encoding: utf-8
class LaborsReport < Prawn::Document

  def set_rep(company, report)
    @company = Company.find(company)
    @report = @company.labors.find(report)
  end
  ...
  Widths = [330, 70, 70, 70]
  # заглавия колонок
  Headers = ['Стаття', 'Код рядка', 'За звітний місяць', 'За період з початку року']
  ...
  def to_pdf
    ...
    move_down(18)
    text "Звіт з праці", :size => 15, :style => :bold, :align => :center
    move_down(10)
